...
------------------------
    ...
    sudo apt install docker.io libfuse2 docker-buildx
...
From docker directory:
    ...
    docker build --network=host -t c3ddocker2204 .

It needs 2 GB of disk space.

The image is based on ubuntu:22.04. In case of old base image (with 404 errors on packages), update with:
...
For automatic compilation and packaging for Linux and Windows, from docker directory:
    ...
    docker run --user "$(id -u):$(id -g)" --rm  -v $(pwd)/..:/c3d c3ddocker2204 /c3d/docker/compile_all.sh
    docker run --rm --device /dev/fuse --privileged -v $(pwd)/..:/c3d c3ddocker2204 /c3d/docker/package_all.sh
...
If something is wrong with files authorizations, try cleaning as docker root :
    ...
    docker run --rm --device /dev/fuse --privileged -v $(pwd)/..:/c3d c3ddocker2204 /c3d/docker/clean_all.sh

For interactive compilation:

    docker run --user "$(id -u):$(id -g)" -ti --rm --device /dev/fuse --privileged -v $(pwd)/..:/c3d c3ddocker2204 bash
